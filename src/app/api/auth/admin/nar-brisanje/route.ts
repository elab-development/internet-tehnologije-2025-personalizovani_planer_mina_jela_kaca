import { db } from "@/db";
import { korisniciTabela, narudzbenicaTabela, stavkaNarudzbeniceTabela } from "@/db/schema";
...
export async function DELETE(req: Request) {
  try {
    ...
    //prvo se brišu sve stavke koje su povezane sa narudžbenicom
    await db.delete(stavkaNarudzbeniceTabela).where(eq(stavkaNarudzbeniceTabela.narudzbenicaID, narID));

    //tek onda se briše narudžbenica
    ...
    console.error(err);
    return NextResponse.json({ error: "Greška pri brisanju narudžbenice" }, { status: 500 });
  }
}