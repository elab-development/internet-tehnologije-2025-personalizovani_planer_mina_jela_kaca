...
export async function POST(req: Request){
    ...
    const [k] = await db.select().from(korisniciTabela).where(eq(korisniciTabela.email, email))
    if(!k){
        return NextResponse.json({error: "Pogresan email ili lozinka!"}, {status: 401})
    }
    ...
    //uzmemo response
    const res = NextResponse.json({id: k.id, email: k.email, ime: k.ime, prezime: k.prezime, adresa: k.adresa}, {status: 200})
